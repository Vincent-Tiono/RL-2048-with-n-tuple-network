...
class Board:
    ...
    def reset(self):
        ...
        self.board[x, y] = 4 if np.random.rand() <= 0.1 else 2
        empty_tiles = self.empty_tiles()
        x, y = random.choice(empty_tiles)
        self.board[x, y] = 4 if np.random.rand() <= 0.1 else 2
    ...
    def act(self, a):
        ...
        if np.array_equal(original, self.board):
    ...
    def merge_to_left(self):
        ...
        for i in range(4):
            ...
            row_reward, new_row = merge(row)
            board_reward += row_reward
