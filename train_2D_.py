...
def compress(grid):
    """Remove 0s from a 2x2 grid row or column"""
    return [[x for x in row if x != 0] for row in grid]


def merge(grid):
    """Merge a 2x2 grid row or column, combining equal values"""
    grid = compress(grid)
    reward = 0
    result = []
